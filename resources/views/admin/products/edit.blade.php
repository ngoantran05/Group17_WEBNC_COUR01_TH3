...
@section('content')
    <h1 class="text-3xl font-bold mb-6 text-gray-800">Sửa Sản phẩm: {{ $product->name }}</h1>
        ...
        <form action="{{ route('admin.products.update', $product->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div>
                    ...
                    <input type="number" step="1000" id="price" name="price" value="{{ old('price', $product->price) }}" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                </div>
                <div>
             ...
             <div class="mb-4">
                ...
                <select id="status" name="status" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                    ...
                    <option value="out_of_stock" @if(old('status', $product->status) == 'out_of_stock') selected @endif>Hết hàng</option>
                </select>
            </div>
            <div class="mb-4">
                <label for="image" class="block text-gray-700 text-sm font-bold mb-2">Ảnh sản phẩm (Để trống nếu không muốn đổi):</label>
                ...
                @if ($product->image)
                    <div class="mt-4">
                        <span class="block text-gray-600 text-sm mb-2">Ảnh hiện tại:</span>
            ...
            <div class="mb-6">
                ...
                <textarea id="description" name="description" rows="5" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">{{ old('description', $product->description) }}</textarea>
            </div>
            <div class="flex items-center justify-end">
                <button type="submit" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline">Cập nhật sản phẩm</button>
            </div>
        </form>
    </div>
@endsection